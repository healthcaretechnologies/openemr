...
include_once("$srcdir/gen_bill_sheet.inc.php");
...
$bat_filename .= (isset($_POST['bn_process_hcfa'])||isset($_POST['bn_bill_sheet_file'])) ? 'pdf' : 'txt';

if (isset($_POST['bn_process_hcfa'])||isset($_POST['bn_bill_sheet_file'])) {
...
if (isset($ar['bn_x12']) || isset($ar['bn_x12_encounter']) || isset($ar['bn_process_hcfa']) || isset($ar['bn_hcfa_txt_file'])|| isset($ar['bn_bill_sheet_file'])) {
...
} else if (isset($ar['bn_process_hcfa']) || isset($ar['bn_hcfa_txt_file'])|| isset($ar['bn_bill_sheet_file'])) {
...
else if (isset($ar['bn_bill_sheet_file'])) {
$log = '';
$lines = gen_bill($patient_id, $encounter, $log);
fwrite($hlog, $log);
$alines = explode("\014", $lines); // form feeds may separate pages
foreach ($alines as $tmplines) {
if ($claim_count++) $pdf->ezNewPage();
$pdf->ezSetY($pdf->ez['pageHeight'] - $pdf->ez['topMargin']);
$pdf->ezText($tmplines, 10, array('justification' => 'left', 'leading' => 12));
}
if (!updateClaim(false, $patient_id, $encounter, -1, -1, 2, 2, $bat_filename)) {
$bill_info[] = xl("Internal error: claim ") . $claimid . xl(" not found!") . "\n";
}
...
if (isset($ar['bn_bill_sheet_file'])) {
fclose($hlog);
// If a writable edi directory exists (and it should), write the pdf to it.
$fh = @fopen($GLOBALS['OE_SITE_DIR'] . "/edi/$bat_filename", 'a');
if ($fh) {
...
<br>
<p>
<h3><?php xl('Billing queue results:','e'); ?></h3>
<a href="billing_report.php">back</a>
<ul>

<?php
if(isset($_POST['bn_bill_sheet']))
{
echo 'Test Bill Sheet';
}
