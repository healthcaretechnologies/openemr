...
// Start Change: 
// Change By : Sanjeewa Gunathilake (HCT) - Sgunathilake@hct.us.com
//Add a new dropdown to show the active medical problems
$myProblems = sqlStatement("SELECT title,diagnosis FROM lists WHERE  " .
  "pid = '$pid' AND type = 'medical_problem' AND diagnosis<>'' AND enddate is null" );

	echo "  <td width='50%' align='center' nowrap>\n";
    echo "   <select style='width:96%' onchange='codeselect(this)'>\n";
    echo "    <option value=''> ".'Current Diagnosis Codes'."</option>\n";
    
while ($row = sqlFetchArray($myProblems)) {
  $title = $row['title'];
  $diagnosis   = str_replace(';','~',$row['diagnosis']);
  $diagnosis = str_replace(':','|',$diagnosis);
  echo "    <option value='$diagnosis'>" . $title . "</option>\n";
}
//End Change.
$last_category = '';
