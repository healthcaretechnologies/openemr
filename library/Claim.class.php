...
class Claim {
	...
	function Claim($pid, $encounter_id) {
		...
		while ($row = sqlFetchArray($res)) {
			...
			foreach ($this->procs as $key => $trash) {
				if (strcmp($this->procs[$key]['code'], $row['code']) == 0 && (strcmp($this->procs[$key]['modifier'], $row['modifier']) == 0 || !$this->using_modifiers)) {
		...
		*****************************************************************/
		...
		$referrer_id = (empty ($GLOBALS['MedicareReferrerIsRenderer']) || $this->insurance_numbers['provider_number_type'] != '1C') ? $this->patient_data['providerID'] : $provider_id;
			...
			foreach ($this->invoice[$code]['dtl'] as $key => $value) {
				if (isset ($value['plv'])) {
					...
					if (isset ($value['pmt']) && $value['pmt'] != 0) {
						...
						if (isset ($value['chg']) && trim(substr($key, 0, 10))) {
					...
					if (preg_match("/^Ins(\d)/i", $value['src'], $tmp)) {
						...
						if (trim(substr($key, 0, 10))) { // not an adjustment if no date
			...
			foreach ($this->invoice[$code]['dtl'] as $key => $value) {
				...
				if ($tmp && $value['pmt'] == 0) { // not original charge and not a payment
					...
					if (preg_match("/Ins adjust $inslabel/i", $rsn, $tmp)) {
						...
						if (preg_match("/To copay $inslabel/i", $rsn, $tmp) && !$chg) {
							$coinsurance = $ptresp; // from manual post
							continue;
						} else
							if (preg_match("/To ded'ble $inslabel/i", $rsn, $tmp) && !$chg) {
								$deductible = $ptresp; // from manual post
								continue;
							} else
								if (preg_match("/$inslabel copay: (\S+)/i", $rsn, $tmp) && !$chg) {
									$coinsurance = $tmp[1]; // from 835 as of 6/2007
									continue;
								} else
									if (preg_match("/$inslabel coins: (\S+)/i", $rsn, $tmp) && !$chg) {
										$coinsurance = $tmp[1]; // from 835 and manual post as of 6/2007
										continue;
									} else
										if (preg_match("/$inslabel dedbl: (\S+)/i", $rsn, $tmp) && !$chg) {
											$deductible = $tmp[1]; // from 835 and manual post as of 6/2007
											continue;
										} else
											if (preg_match("/$inslabel ptresp: (\S+)/i", $rsn, $tmp) && !$chg) {
												continue; // from 835 as of 6/2007
											} else
												if (preg_match("/$inslabel adjust code (\S+)/i", $rsn, $tmp)) {
													$rcode = $tmp[1]; // from 835
												} else
													if (preg_match("/$inslabel/i", $rsn, $tmp)) {
														// Take the defaults.
													} else
														if (preg_match('/Ins(\d)/i', $rsn, $tmp) && $tmp[1] != $insnumber) {
															...
															if ($insnumber == '1') {
																if (preg_match("/\$\s*adjust code (\S+)/i", $rsn, $tmp)) {
																	...
																	if ($chg) {
																		...
																		if (preg_match("/Co-pay: (\S+)/i", $rsn, $tmp) || preg_match("/Coinsurance: (\S+)/i", $rsn, $tmp)) {
																			$coinsurance = 0 + $tmp[1]; // from 835 before 6/2007
																			continue;
																		} else
																			if (preg_match("/To deductible: (\S+)/i", $rsn, $tmp)) {
	...
	function billingContactPhone() {
		if (preg_match("/([2-9]\d\d)\D*(\d\d\d)\D*(\d\d\d\d)/", $this->billing_facility['phone'], $tmp)) {
	...
	function providerNumberType($prockey = -1) {
		$tmp = ($prockey < 0 || empty ($this->procs[$prockey]['provider_id'])) ? $this->insurance_numbers : $this->procs[$prockey]['insurance_numbers'];
	...
	function insuredRelationshipText($ins = 0) {
		return strtoupper($this->payers[$ins]['data']['subscriber_relationship']); // should not happen
	}
	...
	function effectiveDate($ins = 0) {
		return x12clean(trim($this->payers[$ins]['data']['date']));
	}
	...
	function insuredPhone($ins = 0) {
		if (preg_match("/([2-9]\d\d)\D*(\d\d\d)\D*(\d\d\d\d)/", $this->payers[$ins]['data']['subscriber_phone'], $tmp))
			return $tmp[1] . $tmp[2] . $tmp[3];
		return '';
	}
	...
	function patientExternalID() {
		return str_replace('-', '', $this->patient_data['pubpid']);
	}
	function patientSex() {
	...
	function patientDeceasedReason() {
		return strtoupper(x12clean(trim($this->patient_data['deceased_reason'])));
	}
	function patientDeceasedDate() {
		return strtoupper(x12clean(trim($this->patient_data['deceased_date'])));
	}
	//Start Change : Add PCP Inforamtion on Bill Sheet -Sanjeewa
	...
	function reasonForVist() {
		return strtoupper(str_replace('-', '', substr($this->encounter['reason'], 0, 10)));
	}
	function priorAuth() {
	...
	function providerLastName($prockey = -1) {
		$tmp = ($prockey < 0 || empty ($this->procs[$prockey]['provider_id'])) ? $this->provider : $this->procs[$prockey]['provider'];
